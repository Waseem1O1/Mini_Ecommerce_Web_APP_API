 ...
 */
(function(n,t){"use strict";function u(t,u){this.element=t;this.settings=n.extend(!0,{},r,u);this._defaults=r;this._name=i;this._ready=!1;this.currentPage=this.settings.page;this.items=n(this.element).find(this.settings.itemsContainer+" "+this.settings.item);this.totalPages=Math.ceil(this.items.length/this.settings.itemsPerPage);this.totalPages<=1?n(this.element).find(this.settings.pager).hide():this.init()}var i="paginga",r={itemsPerPage:3,itemsContainer:".items",item:"> div",page:1,nextPage:".nextPage",previousPage:".previousPage",firstPage:".firstPage",lastPage:".lastPage",pageNumbers:".pageNumbers",maxPageNumbers:!1,currentPageClass:"active",pager:".pager",autoHidePager:!0,scrollToTop:{offset:15,speed:100},history:!1,historyHashPrefix:"page-"};n.extend(u.prototype,{init:function(){var n,i;this.bindEvents();this.showPage();this.settings.history&&(n=this,t.location.hash&&(i=parseInt(t.location.hash.substring(n.settings.historyHashPrefix.length+1),10),i<=n.totalPages&&i>0&&(n.currentPage=i,n.showPage.call(n))),t.addEventListener("popstate",function(t){n.currentPage=t&&t.state&&t.state.page?t.state.page:n.settings.page;n.showPage.call(n)}));this._ready=!0},bindEvents:function(){var t=this,i=n(t.element),r=i.find(t.settings.previousPage),u=i.find(t.settings.nextPage),f=i.find(t.settings.firstPage),e=i.find(t.settings.lastPage);r.on("click",function(){t.showPreviousPage.call(t)});u.on("click",function(){t.showNextPage.call(t)});f.on("click",function(){t.showFirstPage.call(t)});e.on("click",function(){t.showLastPage.call(t)})},showPreviousPage:function(){this.currentPage--;this.currentPage<=1&&(this.currentPage=1);this.setHistoryUrl();this.showPage()},showNextPage:function(){this.currentPage++;this.currentPage>=this.totalPages&&(this.currentPage=this.totalPages);this.setHistoryUrl();this.showPage()},showFirstPage:function(){this.currentPage=1;this.setHistoryUrl();this.showPage()},showLastPage:function(){this.currentPage=this.totalPages;this.setHistoryUrl();this.showPage()},showPage:function(){var h=this.currentPage*this.settings.itemsPerPage-this.settings.itemsPerPage,w=h+this.settings.itemsPerPage,y,o,u,f,s,e,p;n.each(this.items,function(t,i){if(t>=h&&t<w)return n(i).show(),!0;n(i).hide()});var i=this,r=n(i.element),c=r.find(i.settings.previousPage),l=r.find(i.settings.nextPage),a=r.find(i.settings.firstPage),v=r.find(i.settings.lastPage);if(i._ready&&i.settings.scrollToTop&&r.offset().top-i.settings.scrollToTop.offset<n(t).scrollTop()&&n("html, body").animate({scrollTop:r.offset().top-i.settings.scrollToTop.offset},i.settings.scrollToTop.speed),this.currentPage<=1?(c.addClass("disabled"),a.addClass("disabled")):(c.removeClass("disabled"),a.removeClass("disabled")),this.currentPage>=this.totalPages?(l.addClass("disabled"),v.addClass("disabled")):(l.removeClass("disabled"),v.removeClass("disabled")),y=r.find(this.settings.pager),o=y.find(this.settings.pageNumbers),o){for(o.html(""),u=1,f=this.totalPages,this.settings.maxPageNumbers&&(s=Math.ceil((this.settings.maxPageNumbers-1)/2),u=Math.max(1,this.currentPage-s),f=Math.min(this.totalPages,this.currentPage+s),f-u<this.settings.maxPageNumbers-1&&(u<=s?f=Math.min(this.totalPages,u+this.settings.maxPageNumbers-1):f>this.totalPages-s&&(u=Math.max(1,f-this.settings.maxPageNumbers+1)))),e=u;e<=f;e++)p=e==this.currentPage?this.settings.currentPageClass:"",o.append("<a href='javascript:void(0);' data-page='"+e+"' class='"+p+"'>"+e+"<\/a>");o.find("a").on("click",function(){i.currentPage=n(this).data("page");i.setHistoryUrl.call(i);i.showPage.call(i)})}},setHistoryUrl:function(){var n=this;n._ready&&n.settings.history&&"pushState"in history&&history.pushState({page:this.currentPage},null,"#"+n.settings.historyHashPrefix+this.currentPage)}});n.fn[i]=function(t){return this.each(function(){n.data(this,"plugin_"+i)||n.data(this,"plugin_"+i,new u(this,t))})}})(jQuery,window,document);$(document).ready(function(){$(document).on("click",".plus",function(){var n=$(this).closest(".qty").find(".count"),n=$(this).siblings(".count"),n=$(this).parent().find(".count");$(n).val(parseInt($(n).val())+1);var u=$(this).closest(".qty").find(".count"),t=$(this).closest("tr").find("[id*=price]"),i=$(this).closest(".row").find("[id*=lblsub]"),r=$(this).closest(".row").find("[id*=lbltotal]");$(i).text(parseFloat($(i).text())+parseFloat($(t).val()));$(r).text(parseFloat($(r).text())+parseFloat($(t).val()))});$(document).on("click",".minus",function(){var n=$(this).closest(".qty").find(".count"),n=$(this).siblings(".count"),n=$(this).parent().find(".count");$(n).val(parseInt($(n).val())-1);var u=$(this).closest(".qty").find(".count"),t=$(this).closest("tr").find("[id*=price]"),i=$(this).closest(".row").find("[id*=lblsub]"),r=$(this).closest(".row").find("[id*=lbltotal]");$(i).text(parseFloat($(i).text())-parseFloat($(t).val()));$(r).text(parseFloat($(r).text())-parseFloat($(t).val()))})});$(function(){$("[id*=btnSend]").on("click",function(){var n=$(this).closest("tr").find("td").eq(1).html().trim(),t=$(this).closest("tr").find("td").eq(2).html().trim(),i=$(this).closest("tr").find("td:eq(5) input[type='text']").val().trim(),r=$(this).closest("tr").find("td:eq(6) input[type='text']").val();$.post("/Product/UpdateCart",{id:n,linetotal:t,price:i,quantity:r},function(){})})});$(function(){var n=function(n,t){var u,r;if(n.files)for(u=n.files.length,i=0;i<u;i++)r=new FileReader,r.onload=function(n){$($.parseHTML("<img>")).attr("src",n.target.result).appendTo(t);$(".gallery img").attr("style","height:110px;width: 110px;padding:2px;border:1px solid blue")},r.readAsDataURL(n.files[i])};$("#fileupload").on("change",function(){n(this,"div.gallery")})});$(document).ready(function(){$(document).on("click","#plusdet",function(){$(".countQty").val(parseInt($(".countQty").val())+1)});$(document).on("click","#minusdet",function(){$(".countQty").val(parseInt($(".countQty").val())-1);$(".countQty").val()==0&&$(".countQty").val(1)})});$("#img_01").ezPlus({tint:!0,tintColour:"#F90",tintOpacity:.5});$("#SearchBar").change(function(){var n=this.value.toLowerCase(),t=$("#myproductList #items").each(function(){var t=$(this).text().toLowerCase();$(this).toggle(t.indexOf(n)!==-1)})});